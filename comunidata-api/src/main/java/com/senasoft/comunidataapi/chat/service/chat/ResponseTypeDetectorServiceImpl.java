 ...
 *
 * <p>Este servicio maneja las respuestas de las funciones de análisis de reportes ciudadanos
 * del sistema ComuniData, transformando los datos en respuestas estructuradas para el frontend.
 *
 * <p>Funciones soportadas:
 * - filterByAge: Filtrado por rango de edad del ciudadano
 * - filterByCity: Filtrado por ciudad
 * - filterByCategory: Filtrado por categoría de problema (salud, educación, ambiente, seguridad)
 * - filterByUrgency: Filtrado por nivel de urgencia
 * - filterByGovernmentAttention: Filtrado por atención del gobierno
 * - filterByReportDate: Filtrado por rango de fechas
 * - filterByZone: Filtrado por zona geográfica
 * - semanticSearch: Búsqueda semántica usando RAG con embeddings
 * - generateReport: Generación de reportes con análisis de IA
 */
@Service
public class ResponseTypeDetectorServiceImpl implements ResponseTypeDetectorService {
     ...
     *
     * <p>NOTA: Para las funciones de ComuniData, los datos son procesados por el ChatService
     * que agrega el contexto enriquecido al prompt antes de enviarlo a GPT-5. Por lo tanto,
     * la mayoría de las funciones retornan respuestas de texto simple que permiten a GPT-5
     * generar la respuesta final con análisis inteligente.
     *
     * <p>El flujo es:
     * 1. Function detectada y ejecutada (ej: filterByAge)
     * 2. Datos obtenidos (List<CitizenReport>)
     * 3. FunctionService crea prompt contextual con estadísticas
     * 4. ChatService envía prompt enriquecido a GPT-5
     * 5. GPT-5 genera análisis inteligente final
     */
     ...
     *
     * <p>Este handler NO genera la respuesta final. Simplemente valida que hay datos y retorna
     * una respuesta simple. El ChatService tomará estos datos, generará estadísticas y contexto,
     * y permitirá que GPT-5 cree el análisis final inteligente.
     */
     ...
     *
     * <p>La búsqueda semántica usa RAG (Retrieval-Augmented Generation) con embeddings de OpenAI
     * y MongoDB Atlas Vector Store para encontrar reportes similares semánticamente.
     */
     ...
     *
     * <p>La función generateReport utiliza el CitizenReportGenerationService para crear un PDF
     * con análisis de IA, gráficos y métricas detalladas. Retorna una URL de descarga.
     */
